...
test_that("new_r's output validates input", {
  ...
  expect_error(r_infin(-1), "`n`.*non-negative")
})
...
test_that("new_r validates input", {
  expect_error(new_r(type = "infin"), "`x`.*missing.*numeric.*data frame")
...
test_that("new_r handles metadata", {
  ...
  expect_named(meta_all(r_infin), c("class", "type", "support", "x_tbl"))
  expect_equal(meta_x_tbl(r_infin), x_infin_x_tbl)
  expect_equal(round(meta_support(r_infin), 2), round(x_infin_support, 2))
