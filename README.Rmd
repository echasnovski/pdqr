...
For situations when only type of pdqr-function matters and not its class (like for transforming and summarizing distributions), there are thin wrappers for `new_d()`:

- `new_discrete(x)` is equivalent to `new_d(x, type = "discrete")`.
- `new_continuous(x)` is equivalent to `new_d(x, type = "continuous")`.
